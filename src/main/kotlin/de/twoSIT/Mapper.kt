...
object Mapper {
     ...
     */
    fun parseFloor(relation: Relation, building: Building) {
        val floor = Floor.fromOsm(relation, allWays) ?: return
    ...
    private fun parseIndoorObject(node: Node, level: Int, building: Building) {
        val indoorObject = IndoorObject(node.id, node.latitude, node.longitude, level, node.additionalTags) ?: return
    ...
    private fun parseRoom(way: Way, level: Int, building: Building) {
        val room = Room.fromOsm(way, level)?: return
    ...
    fun exportBuildings() {
        ...
        for (building in buildings) {
            val resultingOsmElements = getContainedElements(building.toOsm())
